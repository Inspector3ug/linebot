...
async function handleEvent(event) {
  try {
    ...
    // If the original message was Chinese, add a pinyin line so English/Indonesian
    // readers can also see how to pronounce it.
    if (detected === 'zh-TW') {
      try {
        const pinyinResult = pinyin(text, { toneType: 'symbol' })
          ...
          .join(' ');
        if (pinyinResult) {
          messages.push({ type: 'text', text: `🔤 Pinyin: ${pinyinResult}` });
          console.log('✅ Pinyin added:', pinyinResult);
        } else {
          console.log('⚠️ Pinyin result was empty for:', text);
        }
      } catch (err) {
        console.log('Pinyin generation failed:', err.message);
      }
    }
